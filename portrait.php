...
<head>
    ...
    <meta name="description" content="Contraste, partager vos portraits en noir et blanc">

    <!-- Titre -->
    <title>Contraste | Portrait</title>
...
<body>
    ...
    <section id="bandeau">
        <div class="container">
            <h1>"Tout portrait qu'on peint avec âme est un portrait non du modèle, mais de l'artiste."</h1><br>
            <p>- Oscar Wilde -</p>
        </div>
    </section>
    ...
    <section class="grid">
        ...
        -->
        <?php
        $reponse = $bdd->query('SELECT content.content_titre, content.content_image_path, content.content_comment, user.user_prenom, categorie.categorie_type FROM content, user, categorie WHERE (content.categorie_id=1 OR content.categorie_id=2) AND content.categorie_id=categorie.categorie_id AND content.user_id=user.user_id ORDER BY RAND() LIMIT 11');
        ?>
        <?php
